...
trade_sheet = pd.read_csv(r'D:\Users\dpsugasa\Desktop\SAN_options/convex_SAN.csv')
...
for q in current_opts:
    d2[q] = d[q]
    temp2[q] = pd.DataFrame(np.nan, columns = ['LAST PRICE_NA'],
                             index = pd.date_range(start = d[q].index[0],
                                                   end = temp[q]['Expiry'][-1],
                                                   freq = 'B'))
    frames = [temp2[q], d[q]]
    d2[q] = pd.concat(frames, join = 'outer', axis = 1)
    d2[q] = d2[q].drop(['LAST PRICE_NA'], axis = 1)
    d2[q].loc[temp[q].index[-1]] = temp[q]['Amount'].\
                                        loc[temp[q].index[-1]]
    d2[q].loc[temp[q].index[0]] = temp[q]['Amount'].\
                                        loc[temp[q].index[0]]
    
    d2[q] = d2[q].fillna(method = 'ffill')
    d2[q] = d2[q].dropna()
    
    d2[q]['trade'] = 0.0
    d2[q]['prev_pos'] = 0.0
    d2[q]['pos'] = 0.0
    d2[q]['trade'].loc[temp[q].loc[temp[q]['Direction'] == 'Buy'].index] = 1.0
    d2[q]['trade'][-1] = -1.0
    d2[q] = d2[q].sort_index()

    for z, row in d2[q].iterrows():
        idx_loc = d2[q].index.get_loc(z)
        prev = idx_loc - 1
        d2[q]['prev_pos'].loc[z] = d2[q]['pos'].iloc[prev]
        if row['trade'] == 1.0:
            d2[q]['pos'].loc[z] = 1.0 + d2[q]['prev_pos'].loc[z]
        elif row['trade'] == -1.0:
            d2[q]['pos'].loc[z] = -1.0 + d2[q]['prev_pos'].loc[z]
        else:
            d2[q]['pos'].loc[z] = d2[q]['prev_pos'].loc[z]

#d2[current_opts]['pos'] = d2[current_opts]['pos_chg'].shift() + d2[current_opts]['trade'] 
#d2[current_opts]['pos'][0] = 0.0 
    
    d2[q]['shares'] = temp[q]['Shares'].iloc[0]
    d2[q]['qtty'] = opt1_qtty
    d2[q]['cash_val'] = 0.0
    d2[q]['trade_val'] = d2[q]['trade']*d2[q]['shares']*\
        d2[q]['qtty']*d2[q]['LAST PRICE']
    d2[q]['pos_val'] = d2[q]['prev_pos']*d2[q]['shares']*\
        d2[q]['qtty']*d2[q]['LAST PRICE']
    
    for z, row in d2[q].iterrows():
        if row['trade'] != 0:
            d2[q]['cash_val'].loc[z] = np.negative(d2[q]['trade'].loc[z]* \
                                    d2[q]['shares'].loc[z]* \
                                    d2[q]['qtty'].loc[z]* \
                                    d2[q]['LAST PRICE'].loc[z])+\
                                    d2[q]['cash_val'].iloc[prev]
                                    
        else:
            d2[q]['cash_val'].loc[z] = d2[q]['cash_val'].iloc[prev] 
        

    d2[q]['total_val'] = d2[q]['trade_val'] +\
                                d2[q]['pos_val'] +\
                                d2[q]['cash_val']
    d2[q] =  d2[q].truncate(after = end_date)
