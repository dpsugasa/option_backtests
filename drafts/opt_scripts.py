...
for i in hist_opts:
    ...
    for z, row in d2[i].iterrows():
        if row['trade'] == 1.0:
            ...
            prev = idx_loc - 1
            d2[i]['pos_chg'].loc[z] = d2[i]['pos_chg'].iloc[prev]
    ...
    d2[i]['day_val'] = d2[i]['pos']*d2[i]['shares']*d2[i]['qtty']*d2[i]['LAST PRICE']
    
    for z, row in d2[i].iterrows():
        if row['trade'] != 0:
            d2[i]['total_val'].loc[z] = np.abs(d2[i]['trade'].loc[z]* \
                                    d2[i]['shares'].loc[z]* \
                                    d2[i]['qtty'].loc[z]* \
                                    d2[i]['LAST PRICE'].loc[z])
        else:
           d2[i]['total_val'].loc[z] =np.abs(d2[i]['pos'].loc[z]* \
                                    d2[i]['shares'].loc[z]* \
                                    d2[i]['qtty'].loc[z]* \
                                    d2[i]['LAST PRICE'].loc[z])               
...
d2[current_opts]['pos_chg'] = 0.0
d2[current_opts]['total_val'] = 0.0
...
for z, row in d2[current_opts].iterrows():
    if row['trade'] == 1.0:
        d2[current_opts]['pos_chg'].loc[z] = 1.0
    elif row['trade'] == -1.0:
        d2[current_opts]['pos_chg'].loc[z] = -1.0
    else:
        idx_loc = d2[current_opts].index.get_loc(z)
        prev = idx_loc - 1
        d2[current_opts]['pos_chg'].loc[z] = d2[current_opts]['pos_chg'].iloc[prev]
...
d2[current_opts]['day_val'] = d2[current_opts]['pos']*d2[current_opts]['shares']*\
    d2[current_opts]['qtty']*d2[current_opts]['LAST PRICE']
    
for z, row in d2[current_opts].iterrows():
    if row['trade'] != 0:
        d2[current_opts]['total_val'].loc[z] = np.abs(d2[current_opts]['trade'].loc[z]* \
                                    d2[current_opts]['shares'].loc[z]* \
                                    d2[current_opts]['qtty'].loc[z]* \
                                    d2[current_opts]['LAST PRICE'].loc[z])
    else:
        d2[current_opts]['total_val'].loc[z] = np.abs( d2[current_opts]['pos'].loc[z]* \
                                    d2[current_opts]['shares'].loc[z]* \
                                    d2[current_opts]['qtty'].loc[z]* \
                                    d2[current_opts]['LAST PRICE'].loc[z])
